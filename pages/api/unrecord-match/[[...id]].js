...
export default async function handler(req, res) {
  try {
    ...
    const maxId = await (await pool.query('SELECT MAX("matchId") FROM matches')).rows[0]['max'];
    const { id } = req.query.id || { id: maxId };
    ...
    res.json({ message: 'Match unrecorded successfully!' });
  } catch (error) {
    console.error(error);
